// PDF BUILDER — Atelier A2: jeder + Häufigkeit
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            ...
            <div class="cast-grid">
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Își descrie rutina</div></div>
    ...
    function buildTheory() {
        ...
        t = t.replace(/<div class="theory-box"\s+style="background:\s*#F5F0E8[^"]*"[^>]*>/g, '<div class="theory-box warn-box">');
        return `<h1 class="chapter">📘 1. Teorie — jeder + Häufigkeit: cât de des?</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillPromptBlock('Übung 1 — Declinarea lui jeder', 'Nom: jeder/jede/jedes · Akk: jeden/jede/jedes · Dat: jedem/jeder/jedem. Capcana: jeden Tag (Akkusativ)!', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += choiceBlock('Übung 2 — Frecvență & jeder', 'Alege varianta corectă (scala immer→nie + jeden Tag).', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillPromptBlock('Übung 3 — Adverbul de frecvență', 'immer → meistens → oft → manchmal → selten → nie. Plus täglich / wöchentlich / monatlich.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillPromptBlock('Übung 4 — jeden Tag / einmal pro Woche', 'Completează cu jeden/jede/jedes/jedem sau cu pro/am.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += translateBlock('Übung 5 — Traducere RO → DE', 'Atenție: jeden Tag (Akk), nie fără nicht, scala de frecvență.', ex5Data);
        }
        return html;
    }
    ...
    function buildVerbs() {
        let html = `<h1 class="chapter new-section">🔁 4. Verbele de rutină — Präsens · Präteritum · Perfekt</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    <div class="speaker">📌 Andreea îți spune:</div>
                    <div>Verbele de rutină merg cu frecvența: aufstehen (jeden Morgen), einkaufen (einmal pro Woche), treffen (jedes Wochenende). Atenție: aufstehen face Perfekt cu SEIN (ist aufgestanden); treffen și anrufen sunt TARI.</div>
                </div>
            </div>`;
